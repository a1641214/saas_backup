...
RSpec.describe ClashRequest, type: :model do
    ...
    describe 'current clash session method' do
        it 'finds the current session of the clash request course component' do
            ...
            expect(clashOne.current_clash_session(comp1)).to eq ("LE01")
            expect(clashOne.current_clash_session(comp2)).to eq ("PR02")
        end
    end
    ...
    describe 'add new request sessions' do
        it 'should add the new sessions into the clash request' do
            ...
            clashOne.course= courseOne
            clashOne.add_new_request_sessions(courseOne, code_hash)
            array = Array.new
            clashOne.sessions.each do |sess|
                array << sess
            end
